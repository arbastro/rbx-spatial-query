...
MODULE LICENSING: This module is licensed under an MIT license; this is a permissive license, meaning it carries very few restrictions in these areas.
...
This can be achieved by using a spatial query to find nearby NPCs within a certain radius of the player.
...
--// CONFIGURATION
SpatialModule.Grid = {}
SpatialModule.CellSize = 50  -- cell size based off game scale

--// MINOR FUNCTIONS
function SpatialModule:Init()
	local bounds = 1000  -- bounds of workspace
	for x = -bounds, bounds, SpatialModule.CellSize do
		SpatialModule.Grid[x] = {}
		for z = -bounds, bounds, SpatialModule.CellSize do
			SpatialModule.Grid[x][z] = {}
		end
	end
end

function SpatialModule:AddObject(obj)
	local cellX = math.floor(obj.Position.X / SpatialModule.CellSize) * SpatialModule.CellSize
	local cellZ = math.floor(obj.Position.Z / SpatialModule.CellSize) * SpatialModule.CellSize
	if not SpatialModule.Grid[cellX] then SpatialModule.Grid[cellX] = {} end
	if not SpatialModule.Grid[cellX][cellZ] then SpatialModule.Grid[cellX][cellZ] = {} end
	table.insert(SpatialModule.Grid[cellX][cellZ], obj)
end

function SpatialModule:RemoveObject(obj)
	local cellX = math.floor(obj.Position.X / SpatialModule.CellSize) * SpatialModule.CellSize
	local cellZ = math.floor(obj.Position.Z / SpatialModule.CellSize) * SpatialModule.CellSize
	if SpatialModule.Grid[cellX] and SpatialModule.Grid[cellX][cellZ] then
		for i, object in ipairs(SpatialModule.Grid[cellX][cellZ]) do
			if object == obj then
				table.remove(SpatialModule.Grid[cellX][cellZ], i)
				return
			end
		end
	end
...
function SpatialModule:GetObjectsInArea(pos, radius)
	local objectsFound = {}
	local startX = math.floor((pos.X - radius) / SpatialModule.CellSize) * SpatialModule.CellSize
	local endX = math.floor((pos.X + radius) / SpatialModule.CellSize) * SpatialModule.CellSize
	local startZ = math.floor((pos.Z - radius) / SpatialModule.CellSize) * SpatialModule.CellSize
	local endZ = math.floor((pos.Z + radius) / SpatialModule.CellSize) * SpatialModule.CellSize

	for x = startX, endX, SpatialModule.CellSize do
		for z = startZ, endZ, SpatialModule.CellSize do
			if SpatialModule.Grid[x] and SpatialModule.Grid[x][z] then
				for _, obj in ipairs(SpatialModule.Grid[x][z]) do
